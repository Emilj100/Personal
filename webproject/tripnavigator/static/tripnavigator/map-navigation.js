...
var map;
var hotelMarker; // Global variabel til hotelmarkøren

// Definér et rødt ikon til hotelmarkøren
  ...
  if (destination) {
    fetch('https://api.opencagedata.com/geocode/v1/json?q=' + encodeURIComponent(destination) + '&key=' + opencage_api_key + '&limit=1')
      ...
      .then(data => {
        ...
        // Giv Leaflet tid til at tegne korrekt
        setTimeout(() => { map.invalidateSize(); }, 100);
        addActivityMarkers(activities, opencage_api_key);

        // Hvis der findes en hoteladresse, tilføj hotelmarkøren
...
function addActivityMarkers(activities, opencage_api_key) {
  ...
  activities.forEach(function(activity) {
    // Kun tilføj markør, hvis der er en gyldig adresse
    if (activity.address &&
...
function addHotelMarker(address, opencage_api_key) {
  // Geokod hoteladresse og tilføj en rød markør
  fetch('https://api.opencagedata.com/geocode/v1/json?q=' + encodeURIComponent(address) + '&key=' + opencage_api_key + '&limit=1')
    ...
    .then(data => {
      if (data && data.results && data.results.length > 0) {
        var lat = data.results[0].geometry.lat;
        var lon = data.results[0].geometry.lng;
        // Hvis der allerede er en hotelmarkør, fjern den
        if (hotelMarker) {
...
// Funktion til at hente CSRF-token fra cookies
...
function updateHotelAddress() {
  ...
  if (newAddress && newAddress.trim() !== "" && newAddress.toLowerCase() !== "none") {
    // Opdater hotelmarkøren på kortet
    addHotelMarker(newAddress, window.mapNavigationContext.opencage_api_key);

    // Send den nye adresse til backend via AJAX POST request
...
function showMessage(message, type) {
  // type kan fx være 'success' eller 'danger' (hvis du bruger Bootstrap)
